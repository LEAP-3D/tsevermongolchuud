...
export default function Sidebar({ activeTab, onChangeTab }: SidebarProps) {
  return (
    <div className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-gray-200/80 md:h-screen md:shrink-0 p-4">
      ...
      <nav className="flex md:block gap-2 md:gap-0 overflow-x-auto md:overflow-visible pb-2 md:pb-0 md:space-y-1">
        {NAV_ITEMS.map(item => (
          <button
            key={item.id}
            onClick={() => onChangeTab(item.id)}
            className={`shrink-0 md:w-full flex items-center gap-2 md:gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
      ...
      <div className="hidden md:block mt-8 px-3 py-4 bg-gray-50 rounded-xl border border-gray-200/80">
